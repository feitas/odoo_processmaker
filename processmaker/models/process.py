...
class PmProcess(models.Model):
    ...
    def _call(self, request, query_param=False, jsonobject=dict(), method='GET'):
        ...
        endresult = ''
        if (method == 'GET'):
            endresult = requests.get(
                pm_url+'/api/1.0/' + request, params=query_param, headers=headers)
        if (method == 'POST'):
            headers.update({'Content-Type': 'application/json'})
            endresult = requests.post(
                pm_url+'/api/1.0/' + request, params=query_param, data=json.dumps(jsonobject), headers=headers)
        if (method == 'PUT'):
            headers.update({'Content-Type': 'application/json'})
            endresult = requests.put(
                pm_url+'/api/1.0/' + request, data=jsonobject, headers=headers)
        if (not bool(endresult) or not endresult.ok):
            raise UserError(str(endresult.status_code) +
                            "-" + str(endresult.content))
    ...
    def _get_starting_activity(self, process_id):
        ...
        starting_activities = self._call(
            'project/%s/starting-tasks' % process_id)
    ...
    def _get_user_of_task(self, process_id, task_id):
        res = self._call('project/%s/activity/%s/assignee' %
                         (process_id, task_id))
        return res
    ...
    def _assign_user_to_task(self, process_id, task_id, user_id):
        ...
        res = self._call('project/%s/activity/%s/assignee' %
                         (process_id, task_id), par, method='POST')
        return res

    @api.model
    def _cancel_case(self, case_id):
        # /cases/{app_uid}/cancel
        res = self._call('cases/%s/cancel' %
                         (case_id.pm_case_id), method='PUT')
        return res
    ...
    def start_process(self, process_id, related_model=False, related_id=False):
        ...
        res = self._call(
            f'process_events/{pm_process_id}', query_param=par, method='POST')
    ...
    def _parse_json(self):
        ...
        """
        _json_record = self.env['ir.attachment'].search(
            [('name', '=', f'{self.name}.json'), ('res_model', '=', self._name), ('res_id', '=', self.id)])
        if not _json_record:
            raise UserError("请通过导出地址跳转到ProcessMaker中下载json文件，并上传到当前记录的附件中。")
        import base64
        self.export_data = base64.b64decode(_json_record.datas).decode(
