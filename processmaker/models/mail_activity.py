...
class MailActivityExt(models.Model):
    ...
    bpm_action = fields.Selection(
        [('pass', '同意'), ('refuse', '拒绝')], string="审批", default='pass')
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            if activity.activity_type_id.is_bpm:
                pm_task = self.env['pm.task'].search(
                    [('odoo_activity_id', '=', activity.id)])
                if not pm_task:
                    raise UserError("找不到相应的工作流任务！")

                pm_task.confirm_case(
                    upload_data={'result': activity.bpm_action})
        ...
        self.with_context({'type': 'direct'}).unlink()
        ...
        if self._context.get('type') != 'direct':
            for activity in self:
                if activity.activity_type_id.is_bpm:
                    ...
                    pm_task.confirm_case(upload_data={'result': 'refuse'})

                    # post message on activity, before deleting it
                    record = self.env[activity.res_model].browse(
                        activity.res_id)
                    record.message_post_with_view(
                        'mail.message_activity_done',
                        values={
